...
function loadImageAsDataUrl(filePath: string): string | null {
  try {
...
export default async function Image({
  ...
  const name = typeInfo?.name ?? '不明'
  const tagline = typeInfo?.tagline ?? ''

  const imagePath = path.join(process.cwd(), 'public', `${name}.png`)
  const characterDataUrl = loadImageAsDataUrl(imagePath)
